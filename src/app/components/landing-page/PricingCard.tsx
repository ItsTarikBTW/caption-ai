...
const PricingCard = ({ type, desc, price, credits, upload, download_speed, video_length, ai_powered, year }: FeaturesCardValues) => {
    return (
        <div className={`text-black p-5 flex flex-col items-center justify-start my-5 rounded-md md:w-[27vw] ${(type == "Standard") ? "border-2 border-[#5D14A6]": (type == "Premium" ? "border-2 border-[#F22E76]" : "")}`}>
            ...
            <div className="text-center py-5 flex-grow-1">
                <span className="font-bold text-5xl">${!year ? price : ((price * 12) * 0.12).toFixed(2)}</span><span>/mounth</span><br />
